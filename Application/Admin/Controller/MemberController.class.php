...
class MemberController extends CommonController {
	public function select(){
        ...
        if($_GET['name']){
		   ...
		   foreach ($users as $k1=>$v1){
                   $utype =M('member_type')->where(array('id'=>$v1['usertype']))->find();   
                   $users[$k1]['usertype']=$utype['typename'];
                   }
    ...
    public function editeUser(){
        ...
        if($_POST && $uid){
            ...
            echo "<script>alert('修改成功');window.location.href = '".__ROOT__."/index.php/Admin/Member/select';</script>";exit();
        }
        $userinfo = $menber->where(array('uid'=>$uid))->select();
        $this->assign('res',$userinfo[0]);
		$member_type = M('member_type');
		$usertype= $member_type->select();
    ...
    public function charge(){
        ...
        if($_POST['user']&&$_POST['num']){
            if($_POST['num']<=0){
                echo "<script>alert('请输入正确金额');window.location.href = '".__ROOT__."/index.php/Admin/Member/charge';</script>";exit();
            }
            $user = $menber->where(array('tel'=>$_POST['user']))->select();
            if(!$user[0]){
                echo "<script>alert('用户不存在');window.location.href = '".__ROOT__."/index.php/Admin/Member/charge';</script>";exit();
            }
            ...
            if($res){
                ...
                echo "window.location.href = '".__ROOT__."/index.php/Admin/Member/charge';";
                echo "</script>";
                exit;
            }
        ...
        if($_POST ){
            ...
            if($_POST['fuid']){
                ...
                if($fuids[0]){
                    ...
                    echo "<script>alert('上级用户不存在');window.location.href = '".__ROOT__."/index.php/Admin/Member/addUser';</script>";
                    $this->display();
                    exit();
                }
            }
            ...
            if($isuser[0]){
                echo "<script>alert('电话已注册');window.location.href = '".__ROOT__."/index.php/Admin/Member/addUser';</script>";
                $this->display();
                exit();
            }
			...
			$data['addtime'] = time();
            $uid =  $menber->add($data);
            $arrfuids=array_filter(explode(",",$fids));
			$holdnum=10;
            ...
            echo "<script>alert('添加成功');window.location.href = '".__ROOT__."/index.php/Admin/Member/select';</script>";exit();
        }
        $this->display();
    }
...
//        }
    ...
    public function userupdate(){
        ...
        if($_POST){
            ...
            if($result){
                echo "<script>alert('更新成功');window.location.href = '".__ROOT__."/index.php/Admin/Member/select';</script>";exit();
            }else{
                echo "<script>alert('更新失败');window.location.href = '".__ROOT__."/index.php/Admin/Member/select';</script>";exit();
            }
        }
...
//    删除用户
    public function userdelete(){
        ...
        if($result){
            echo "<script>window.location.href = '".__ROOT__."/index.php/Admin/Member/select';</script>";exit();
        }else{
            echo "<script>alert('更新失败');window.location.href = '".__ROOT__."/index.php/Admin/Member/select';</script>";exit();
        }
    }
...
//                $chargebag =$user[0]['chargebag']+$result[0]['income'];
                ...
                echo "<script>alert('更新成功');window.location.href = '".__ROOT__."/index.php/Admin/Member/chargesheng';</script>";exit();
            }
            if($_GET['state']==2){
                $data['type'] =2;
                $data['state'] =0;
                $income->where(array('id'=>$_GET['id']))->save($data);
                echo "<script>window.location.href = '".__ROOT__."/index.php/Admin/Member/chargesheng';</script>";exit();
            }
        }
    }
...
//        print_r($result);die;
    ...
    public function tixiandetail(){
        if($_POST){
            ...
            echo "<script>window.location.href = '".__ROOT__."/index.php/Admin/Member/istixian/id/".$_GET['id']."/state/".$_POST['state']."';</script>";exit();
        }
        $income =M('incomelog');
...
//                    echo "<script>alert('钱包余额不足，手续费不能扣除');window.location.href = '".__ROOT__."/index.php/Admin/Member/tixiansheng';</script>";exit();
//                }
...
//                $income->add($data);
                ...
                echo "<script>alert('suceess');window.location.href = '".__ROOT__."/index.php/Admin/Member/tixiansheng';</script>";exit();
            }
            if($_GET['state']==2){   // 提现失败
                ...
                $data['state'] =3;
                $income->where(array('id'=>$_GET['id']))->save($data);
                echo "<script>window.location.href = '".__ROOT__."/index.php/Admin/Member/tixiansheng';</script>";exit();
            }
        }
    }
	...
	public function usertype()
	{
        ...
        if($_GET['typename']){
            ...
            $usertype= $member_type->where($map)->select();
        }else{
            $usertype= $member_type->select();
        }
        $this->assign('usertype',$usertype);
	 ...
	 public function addusertype(){
        if($_POST ){
            ...
            $isuser= $member_type->where(array('typename'=>$_POST['typename']))->select();
            if($isuser[0]){
                echo "<script>alert('该职位已存在');window.location.href = '".__ROOT__."/index.php/Admin/Member/addusertype';</script>";
                $this->display();
                exit();
            }
            $data =$_POST;
            $uid =  $member_type->add($data);

            echo "<script>alert('添加成功');window.location.href = '".__ROOT__."/index.php/Admin/Member/usertype';</script>";exit();
        }
        $this->display();
    }
	...
	public function editusertype(){
        ...
        if($_POST){
            $data=$_POST;
            $result = $member_type->where(array('id'=>$_GET['id']))->save($data);
            if($result){
				
                //echo "<script>alert('更新成功');window.location.href = '".__ROOT__."/index.php/Admin/Member/usertype';</script>";
				$this->display("Common:message");
				exit();
            }else{
				$this->display("Common:message");
				exit();
                echo "<script>alert('更新失败');window.location.href = '".__ROOT__."/index.php/Admin/Member/usertype';</script>";exit();
            }
        }
        $type= $member_type->where(array('id'=>$_GET['id']))->select();
        $this->assign('res',$type[0]);
        $this->display();
    }
